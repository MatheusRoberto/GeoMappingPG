    ...
    while (i <= j):
        ...
        for d in data:
            ...
            try:
                ...
                fichaTecnica = re.html.find('body > div:nth-child(9) > div.imovelv2.imovelToSell > div > div.imovelInfoMain > div.p60.imobInfoBox.correctAlign > div:nth-child(1) > h5 > i', first=True)
                if fichaTecnica is not None:
                    ...
                    result = next((el for el in element if 'Vila' in str(el)), None)
                    if result is not None:
                        vila = cleanhtml(str(result))
                        vila = extractVila(vila)

                    result = next((el for el in element if 'Cidade' in str(el)), None)
                    if result is not None:
            ...
            if end and preco:
                ...
                if precoAnuncio > 0 and pont >= 40:
                    if not bairro and not vila and not cidade: 
                        (rua, numero, bairro, vila, cidade) = estruturandoEndereco(end.text)
                    else:
...
def main():
    ...
    r = session.get('https://www.conceitoimoveispg.com.br/busca/venda/cidade_ponta-grossa')
    soup = BeautifulSoup(r.text, 'html.parser')

    npag = cleanhtml(str(soup.select('body > div.section.toSell > div > div.searchResults.searchResults3Col > div.paginacao > ul > li:nth-child(6) > a')))
    npag = npag.replace('[', '')
    npag = npag.replace(']', '')
    n = int(int(npag) / 4)
